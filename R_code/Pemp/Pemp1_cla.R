...
mz_tunja <- mz_boyaca[mz_boyaca$MPIO_CCDGO==15001,]
uptc<- st_read(dsn = "./UPTC/SHP_CATASTRO FIN/", layer = "SHP_CONVENIO UPTC")
ciudad<- st_read(dsn = "./pemp. centro historico/4_INFO_CARTOGRAFICA_F/SHAPES/", layer = "BASE PREDIOS TUNJA")
ciudad2<- st_read(dsn = "./pemp. centro historico/4_INFO_CARTOGRAFICA_F/SHAPES/", layer = "base_predios_tunja_sep_2010")



ch <- st_read(dsn ="./pemp. centro historico/4_INFO_CARTOGRAFICA_DG/SHAPES/" ,layer = "Delimitacion_CH_vigente")
ggplot(data = mz_tunja)+
  geom_sf()
view(predios)
view(uptc)
actuimpuesto <- read_excel("Juan Pablo - Mapas/actuimpuesto.xlsx")
actuimpuesto2 <- read_excel("Juan Pablo - Mapas/actuimpuesto.xlsx")
noactuimpuesto <- read_excel("Juan Pablo - Mapas/noactuimpuesto.xlsx")

actualizado2019 <- read_excel("Juan Pablo - Mapas/actualizado2019.xlsx")


impuesto_actual2 <- uptc %>% 
  left_join(actuimpuesto2)

impuesto_antes <- uptc %>% 
  left_join(noactuimpuesto)

ggplot(impuesto_antes %>% filter(Impuesto < 10000000000)) +
  geom_sf(aes(fill = Impuesto))



ggplot(mz_tunja, fill = "white")+
geom_sf(data=impuesto_actual %>% filter(Impuesto < 100000000),aes(fill = Impuesto), color = "white")+
geom_sf(data = ch, fill = "yellow")+ geom_sf(data = mz_tunja)



quantile(actualizado2019$VALOR_PAGADO, prob = c(0.01, 0.25, 0.5, 0.75, 0.9,0.99))          # 10 partes,                                                
...
mz_tunja <- mz_boyaca[mz_boyaca@data$MPIO_CCDGO==15001,]
length(mz_tunja)

"Dejemos unicamente las manzanas de la zona urbana"
table(mz_tunja@data$CLAS_CCDGO)
mz_tunja <- mz_tunja[mz_tunja@data$CLAS_CCDGO==1,]
plot(mz_tunja)

# Calculando los centroides de cada poligono
"(Explicar intuicion con circunferencia)"
cen_mz_tunja <- gCentroid(mz_tunja,byid = T)
plot(cen_mz_tunja , col="red",cex=0.1,pch=19)

"Pintando la capa de manzanas y la capa de centroides"
plot(mz_tunja,col="gray")
plot(cen_mz_tunja , col="red", cex=0.1 , add=T,pch=19)
...
for ( i in c(cen_mz_tunja,mz_tunja,mz_boyaca)){
        print(proj4string(i))
}
"Reproyectemelos"
...
cen_mz_tunja <- reproyectar(cen_mz_tunja)
mz_tunja <- reproyectar(mz_tunja)
mz_boyaca <- reproyectar(mz_boyaca)
...
turismo <- readOGR(dsn ="./pemp. centro historico/4_INFO_CARTOGRAFICA_DG/SHAPES/" ,layer = "Turismo")
turismo
turismo <- reproyectar(turismo)
turismo@data$id_barrio <- seq(1,length(turismo),1)

ch <- readOGR(dsn ="./pemp. centro historico/4_INFO_CARTOGRAFICA_DG/SHAPES/" ,layer = "Delimitacion_CH_vigente")
#ch <- readOGR(dsn ="./pemp. centro historico/4_INFO_CARTOGRAFICA_DG/SHAPES/" ,layer = "base_predios_tunja_4_mar_10")
...
plot(ch)
View(ch@data) 
# Filtremos el turismo
table(turismo@data)
plot(mz_tunja)
plot(turismo,col="blue",pch=19,add=T)
plot(ch,col="pink",add=T)





predios <- readOGR(dsn ="./pemp. centro historico/4_INFO_CARTOGRAFICA_DG/SHAPES/" ,layer = "PREDIOS_AI")


table(Catastro$codigo)

library(haven)
Catastro <- read_dta("Juan Pablo - Mapas/Datasets/Catastro.dta")
ch@data$Shape_Leng <- Catastro$st_area_sh
dim(Catastro)
dim(Catastro)
names(Catastro)

Catastro = rename(Catastro, c(Shape_Leng="st_area_sh"))




Catastro <- as.data.frame(Catastro)
