...
uber_tweets1 <- search_tweets("tourism", n = 1000000, include_rts = FALSE, lang="en", geocode =  "45.01627,-93.26566,2000km",time = "2005-01-01 2019-10-30")#localizacion del centro de Minneapolis, USA
uber_tweets2 <- search_tweets("governance", n = 1000000, include_rts = FALSE, lang="en", geocode =  "45.01627,-93.26566,2000km", time = "2005-01-01 2019-10-30")
...
write.csv(a, file="turnor.csv")

tw1 <- search_tweets("tourism", n = 1000000, include_rts = TRUE, lang="en", geocode =  "45.01627,-93.26566,2000km",time = "2005-01-01 2019-10-30")#localizacion del centro de Minneapolis, USA
a<- tw1 %>% 
  filter(!is_retweet) %>% 
  filter(retweet_count == max(retweet_count)) %>% 
  select(screen_name, retweet_count, followers_count, location, text)
write.csv(a, file="turnor2.csv")
...
a<-uber_tweets2 %>% 
  ...
  arrange(desc(followers_count)) %>% 
  select(screen_name, followers_count, location, text)
write.csv(a, file="gobnor.csv")

tw2 <- search_tweets("governance", n = 1000000, include_rts = TRUE, lang="en", geocode =  "45.01627,-93.26566,2000km", time = "2005-01-01 2019-10-30")
a<- tw2 %>% 
  filter(!is_retweet) %>% 
  filter(retweet_count == max(retweet_count)) %>% 
  select(screen_name, retweet_count, followers_count, location, text)
write.csv(a, file="gobnor2.csv")
...
Categoria <- rep("Tourism" , 4772)
uber_tweets11 <- data.frame(uber_tweets1,Categoria)
Categoria <- rep("Governance" , 4692)
